...
extension UINavigationBar
{
    ...
    fileprivate func wr_setBackgroundImage(image:UIImage)
    {
        ...
        if (backgroundImageView == nil)
        {
            ...
            backgroundImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: Int(bounds.width), height: Int(NAVIHEIGHT)))
...
//            subviews.first?.insertSubview(backgroundImageView ?? UIImageView(), at: 0)
            if (self.subviews.count > 0) {
                ...
                self.insertSubview(self.backgroundImageView ?? UIImageView(), at: 0)
            }
    ...
    fileprivate func wr_setBackgroundColor(color:UIColor)
    {
        ...
        if (backgroundView == nil)
        {
            // add a image(nil color) to _UIBarBackground make it clear
            setBackgroundImage(UIImage(), for: .default)
            backgroundView = UIView(frame: CGRect(x: 0, y: 0, width: Int(bounds.width), height: Int(NAVIHEIGHT)))
            ...
            if (self.subviews.count > 0) {
                ...
                self.insertSubview(backgroundView ?? UIView(), at: 0)
            }
        }
    ...
    fileprivate func wr_setBackgroundAlpha(alpha:CGFloat)
    {
        ...
        if #available(iOS 11.0, *) {
            ...
            for view in self.subviews {
                if String(describing: type(of: view)).contains("UIbarBackGround") {
                    view.alpha = alpha
                }
            }
            
            // iOS 下如果不设置 UIBarBackground 下的UIView的透明度，会显示不正常
            if let firstView = barBackgroundView.subviews.first {
