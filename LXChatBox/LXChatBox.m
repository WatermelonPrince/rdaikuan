...
- (void)changeFrame:(CGFloat)height{
    ...
    if (self.status == LXChatBoxStatusShowFace || self.status == LXChatBoxStatusShowMore) {
        totalH = height + BOXTEXTViewSPACE *2 + BOXOTHERH + (KScreenH < 812 ? 0 : 34) ;
        if (keyboardY ==0) {
            keyboardY = KScreenH;
        }
        ...
        self.moreButton.y =  self.faceButton.y = self.voiceButton.y  = totalH - BOXBTNBOTTOMSPACE- CHATBOX_BUTTON_WIDTH-BOXOTHERH - (KScreenH < 812 ? 0 : 34);
    ...
    switch (_status) {
        case LXChatBoxStatusNothing:
        {
            ...
            [UIView animateWithDuration:0.3 animations:^{
                self.frame = CGRectMake(0, KScreenH - self.textView.height - 2 *BOXTEXTViewSPACE- (KScreenH < 812 ? 0 : 34), KScreenW , self.textView.height + 2 *BOXTEXTViewSPACE + (KScreenW < 812 ? 0 : 34));
        ...
        case LXChatBoxStatusShowFace:
        {
            ...
            self.height = self.textView.height+2 *BOXTEXTViewSPACE + BOXOTHERH + (KScreenH < 812 ? 0 : 34);
    ...
    if (!_isDisappear) {
        ...
        [UIView animateWithDuration:duration animations:^{
            ...
            if (keyboardF.origin.y > KScreenH) {
                self.y = KScreenH- self.height  - (KScreenH < 812 ? 0 : 34);
            }else
            {
                self.y = keyboardF.origin.y - self.height - (KScreenH < 812 ? 0 : 34);
            }
                        NSLog(@"%f",self.y);
        }];
        if ([self.delegate respondsToSelector:@selector(changeStatusChat:)]) {
